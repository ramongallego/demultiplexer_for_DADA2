...
path1 <- params$folder
dir.create(file.path(path1, "filtered"))
```
Firstly, we'll find the patterns that separate our Fwd, Rev, .1 and .2 files. look at the quality of the .1 and .2 reads
```{r listing files}
F1s <- tibble (F1s = sort(list.files(path1, pattern="_Fwd.1.fastq", full.names = TRUE))) |>
  mutate (fastq_header = str_replace(basename(F1s), "_Fwd.1.fastq","") )
               
F2s <- tibble (F2s = sort(list.files(path1, pattern="_Fwd.2.fastq", full.names = TRUE))) |> 
  mutate (fastq_header = str_replace(basename(F2s), "_Fwd.2.fastq","") )

R1s <- tibble (R1s = sort(list.files(path1, pattern="_Rev.1.fastq", full.names = TRUE))) |> 
  mutate (fastq_header = str_replace(basename(R1s), "_Rev.1.fastq","") )

R2s <- tibble(R2s = sort(list.files(path1, pattern="_Rev.2.fastq", full.names = TRUE))) |> 
  mutate (fastq_header = str_replace(basename(R2s), "_Rev.2.fastq","") )

inner_join(F1s, F2s) |> 
  inner_join(R1s) |> 
  inner_join(R2s) |> 
  inner_join(sample.map) -> files

files |> 
  mutate(across(.cols= c(F1s, F2s, R1s, R2s), 
                .fns = ~ file.path(path1, "filtered", basename(.x)),))
```
...
```{r qplot1, echo=FALSE}
plotQualityProfile(files$F1s[1:4])
```
...
out_Fs_tibble |> write_csv(file.path(params$folder, "outF.csv"))
out_Rs_tibble |> write_csv(file.path(params$folder, "outR.csv"))
