...
inner_join(sample.map[,2:3],seqs_tidy,by = c("fastq_header" = "sample")) %>% 
  select(-fastq_header) %>%
  write_csv(paste0(path1,"/DUP_table.csv"))
...
track <- as.data.frame(cbind(out, sapply(dadaFs, getN), sapply(mergersF, getN), rowSums(seqtab), rowSums(seqtab.nochim)))
# If processing a single sample, remove the sapply calls: e.g. replace sapply(dadaFs, getN) with getN(dadaFs)
colnames(track) <- c("input", "filtered", "denoised", "merged", "tabled", "nonchim")
track$sample <- good.sample.names
inner_join(sample.map[,2:3],track, by = c("fastq_header"="sample")) %>%
  select(-fastq_header) %>%
  write_csv(paste0(path1,"/dada2_summary.csv"))
