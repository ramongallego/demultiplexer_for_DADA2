...
```
Now a check on the direction of reads - I do it after writing the files so I can still use the output.
...
cat seqnames.txt | cut -c 1-120 | xargs -I '{}' sed -n 's/{}.*$//p' "${READ1}" | awk 'length($0)>15' > "${F1_found}"

cat seqnames.txt | cut -c 1-120 | xargs -I '{}' sed -n 's/{}.*$//p' "${READ2}" | awk 'length($0)>15' > "${F2_found}"
